...
# Nếu py -3.11 báo "No suitable Python runtime found"
# nhưng lệnh python --version vẫn chạy được:
python -m venv .venv
.\.venv\Scripts\Activate.ps1
...
Nếu PowerShell chặn `Activate.ps1`, chạy trong terminal hiện tại:
...
Hoặc không cần activate, gọi thẳng Python trong `.venv`:
...
`.venv` là thư mục sinh ra tại máy local, không commit lên GitHub. Khi clone dự án ở máy khác, chỉ cần tạo lại `.venv` và cài dependency từ `requirements.txt`.
...
File `requirements.txt` đã gộp đủ thư viện cho app desktop, AI, API Flask tùy chọn và chạy test.

### 4. Cấu hình API (tùy chọn)

Tạo file `.env` tại thư mục gốc, có thể copy từ `.env.example`:
...
├── requirements.txt            # Toàn bộ thư viện cần cài
...
**Yêu cầu:** Các thư viện AI đã nằm trong `requirements.txt`.

Lần đầu chạy tự động tải TinyLlama (~600 MB). Có thể fine-tune thêm bằng dữ liệu của người dùng:

1. Trong chatbot, nhấn **Train model**
2. App tạo dữ liệu Q&A từ lịch sử giao dịch và train DistilGPT2 (~5-15 phút)
